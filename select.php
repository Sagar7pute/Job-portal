...
<div class="dashboard" style="position: fixed;width: 18%;height: 100%;background:#222D32">
 ...
 //$res=$con->query("SELECT job.jid,employee.eeid,eename,eadd,econ,STATUS FROM `job`,employee,application WHERE job.jid=application.jid AND employee.eeid=application.eeid AND STATUS=0 AND jid='$jid' AND job.eid='$id'");
 $res=$con->query("SELECT job.jid,employee.eeid,eename,skill,eadd,econ,STATUS FROM `job`,employee,application WHERE job.jid=application.jid AND employee.eeid=application.eeid AND STATUS=0 AND job.jid='$jid' AND job.eid='$id'");
      ...
      <?php $i=0;
        ...
        ?>
		 ...
		 <td><?php echo $row['skill']; ?></td>
            ...
            <td colspan="center"><a href="reject.php?eeid=<?=$row['eeid']?>&jid=<?=$row['jid']?>"><button name="rej" class='btn btn-danger'>Reject</button></a></td>
            <td colspan="center"><a href="approve.php?eeid=<?=$row['eeid']?>&jid=<?=$row['jid']?>"><button name="approve" class='btn btn-success'>Approve</button></a></td>
		</tr>
      <?php
		}
		?>
